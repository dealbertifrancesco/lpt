...
#'   (functions), \code{true_B} (scalar), \code{true_B_d} (scalar), \code{dgp}.
#'
#' @keywords internal
simulate_lpt <- function(n = 1000, n_pre = 3, n_post = 1,
  ...
  # Selection function: component of E[Y_0(t)|D=d] that varies with dose
  # no_selection:           mu(d,t) = 0               -> true B = 0
  # linear_selection:       mu(d,t) = rho * d         -> true B = |rho|, constant
  # time_varying_selection: mu(d,t) = rho*d + rho2*d*t -> mu'(d) varies with t
  selection_fn <- switch(dgp,
    "no_selection"           = function(d, t) rep(0, length(d)),
    "linear_selection"       = function(d, t) rho * d,
    "time_varying_selection" = function(d, t) rho * d + rho2 * d * t
  )

  # Treatment effect (same for all DGPs)
  ...
  true_B <- switch(dgp,
    ...
    "time_varying_selection" = abs(rho) + abs(rho2) * n_periods
  )

  true_B_d <- switch(dgp,
    "no_selection"           = 0,
    "linear_selection"       = abs(rho),
    "time_varying_selection" = abs(rho2)
  )
